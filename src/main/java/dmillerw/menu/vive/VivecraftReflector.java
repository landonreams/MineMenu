...
/**
 * This class allows for additional ViveCraft functionality without a hard dependency.
 * Borrowed from techjar's VivecraftReflector
 * https://github.com/Techjar/VivecraftForgeExtensions/
 */
...
public class VivecraftReflector {
    ...
    static {
        try {
            ...
            installed = true;
            LogHandler.debug("Vivecraft is installed.");
        } catch (ClassNotFoundException ex) {
            installed = false;
            LogHandler.debug("Vivecraft is not installed.");
        }
        if (installed) {
            try {
                Class.forName("com.mtbs3d.minecrift.api.IRoomscaleAdapter");
                newAPI = true;
                LogHandler.debug("Vivecraft is new API.");
            } catch (ClassNotFoundException ex) {
                newAPI = false;
                LogHandler.debug("Vivecraft is old API.");
            }
        }
    }
    ...
    @SneakyThrows(Exception.class)
    public static Vec3d getHeadPosition() {
        ...
        if (newAPI) {
            ...
            return (Vec3d)method_getHMDPos_World.invoke(getVRPlayer());
        } else {
            if (method_getCameraLocation == null) {
                method_getCameraLocation = EntityRenderer.class.getDeclaredMethod("getCameraLocation");
            }
            return (Vec3d)method_getCameraLocation.invoke(Minecraft.getMinecraft().entityRenderer);
        }
    }
     ...
     */
    @SneakyThrows(Exception.class)
    public static Vec3d getControllerPositon(int controller) {
        if (newAPI) {
            ...
            return (Vec3d)method_getControllerPos_World.invoke(getVRPlayer(), controller);
        } else {
            ...
            return (Vec3d)method_getAimSource.invoke(getAimController(), controller);
        }
    }
